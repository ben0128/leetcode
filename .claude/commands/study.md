...
4. **存檔到 `study/plan_{nn}_{YYYY-MM-DD}.md`**
   - 包含計畫內容和進度追蹤表格（初始狀態全部為 `—`）
   - 編號接續 `study/` 資料夾中最大的編號

5. **問學生要從哪題開始**，然後以 `/practice` 助教模式引導該題：
   ...
   - Follow the full practice flow (progressive hints, testing, complexity discussion)
   - After solving, commit the solution file

6. **每完成一題，立即更新** `study/plan_{nn}_{YYYY-MM-DD}.md` 的進度欄位：
   - 結果：✅ = 獨立解出最優解 / ⚠️ = 需要提示或非最優 / ❌ = 無法解出
   - 花費時間
   - 筆記（掙扎點或收穫）

7. **全部 5 題完成後**，append 摘要到 `./ANALYSIS.md` 的「讀書計畫紀錄」section：
