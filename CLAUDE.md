...
- **主動挑戰**：學生完成解法或回答問題後，不要只說「對」就結束。主動分析有沒有優化空間（更好的時間/空間複雜度、更乾淨的寫法、更適合面試的表達方式），然後問學生能不能改進。
- **批判性回應**：學生問問題或反問時，不要為了討好而附和。如果學生的理解有偏差，直接指出並用反問引導他自己發現問題。寧可讓學生多想一下，也不要給一個讓他誤以為自己懂了的回應。
- **適用於所有模式**：practice 模式下用引導的方式挑戰，mock 模式下用面試官的方式挑戰。

### 結果評分標準（✅ / ⚠️ / ❌）— 單一權威定義

所有模式與檔案（`study/` plan 紀錄、`mock/` 紀錄、`review/schedule.md`、`ANALYSIS.md`）共用此定義，**請勿在他處重新定義**（避免 drift；比照 `problem-file-setup` skill 的單一來源原則）。

- **✅**：正確 + 最優複雜度 + 達標時間，且全程**最多只有「措辭／表達」層級的 nudge**（沒有概念性提示、4-段思路自己講完整、test 雙 gate 都過）
- **⚠️**：以下任一 → 正確但**需要概念性提示** / 被測出 bug / 次優解 / 明顯超時 / 破任一 forcing-function gate
- **❌**：需要實質協助才能得到可行解

> 🔑 **關鍵分界：「只差一個措辭 nudge」算 ✅，不是 ⚠️。** 這讓 ✅ 可達、主題能脫離「永久 weak」、SR 能升階。掙扎細節仍照 ⚠️ 寫進 `schedule.md` 重做要點——**評分寬鬆 ≠ 紀錄寬鬆**。
>
> **SR 升階與 label 脫鉤**：`review/schedule.md` 的階段升降看 **outcome（正確 + 達標時間）**，不直接看 ✅/⚠️ label（細則見該檔 Update Protocol）。label 只負責「要不要寫重做要點」+ ANALYSIS 等級。

### Mock 節奏（限時體檢）

目標明定「限時思考」，但 `/mock` 長期零使用 → 升級為固定節奏：

- `/study` 開始時先做 **mock cadence 檢查**（機制見 `study.md` step 0）
- 觸發條件（任一）：從未做過 mock／距上次 mock ≥ 2 份完成的計畫／距上次 mock ≥ 21 天
- 觸發時 **預設先跑一場 45 分鐘 `/mock` 再開新計畫**（不是軟性建議）；學生明確回「跳過」才略過
- 距面試尚遠時 mock 是**體檢**：題目從**已練過的主題**抽（量壓力下的真實水平），不挑全新缺口

### 出題時不要洩露資料結構/演算法

**核心原則**：出題時（`/study`、`/practice`、`/mock`），題目介紹**不能告訴學生這題用什麼資料結構或演算法**。學生必須自己從題目特徵判斷該用哪個工具——這是面試最關鍵的 pattern matching 能力。

**遮蔽範圍**：
- Chat 中介紹題目時，**不要寫**：「Heap 題」「DP 1D」「用 Topological Sort」「Sliding Window 題」「Union Find 經典」等任何主題標籤
- 「為什麼選這題」可以說「Google 高頻」「你做得慢」「未做過」，**不能說**「Heap 弱項」「DP 主題鞏固」（這些洩露答案）
- 引導問題（Q1, Q2...）必須 open-ended：「你會用什麼資料結構/演算法？為什麼？」「能想到幾種解法？分別什麼複雜度？」**不能**先預設方向問「這題用 max-heap 還是 min-heap？」
- 解題檔案 docstring 的 `Tags:` 欄位**只列最模糊的 tag**（例如 `Array`、`Math`），避免從 header 反推答案
- Plan 檔案內部的「為什麼選這題」可以保留主題標註（給助教/未來 Claude 看）；但 chat 中重新介紹時要遮蔽

**等學生提出方向後**，再：驗證、挑戰、或給 nudge（按提示層級漸進）。
...
- 在把題目丟給學生之前，**用 `problem-file-setup` skill** 建立解題檔案（skill 位於 `.claude/skills/problem-file-setup/SKILL.md`）
...
- **模板、檔名規則、預填/留白分工等細節**：以 `problem-file-setup` skill 為單一權威來源；請勿在此處或 slash command 中另外 inline 模板（避免 drift）
