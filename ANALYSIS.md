...
| BST | 4 | ⚠️⚠️ | 2d | **weak** | **235 LCA SR-burndown 重做 ⚠️（Plan #11 #2, 7/2）＝依預告規則視同 Fail 重置 2d**：BST ordering 下沉首次零提示自己講出（chronic 半收），但 iterative O(1) 仍需引導＋**口頭答迴圈卻寫出遞迴**（模型→code 翻譯第 4 次）；invariant 迴避 3 輪、「含 curr」措辭洞＋「split 為何最低」循環論證均助教收尾；docstring gate 破（TODO 收工）。亮點：swap + inclusive split 一次寫對、自加跨側 test。230 Kth ⚠️ |
